...
describe("ReadonlyDictionary", () => {
    ...
    describe("#containsKey()", () => {
        it("should return true if the dictionary contains the given key", () => {
            const dictionary = new ReadonlyDictionary(new Dictionary(null, Enumerable.range(1, 100).toDictionary(x => x, x => x)));
            for (let i = 0; i < 100; i++) {
    ...
    describe("#keys()", () => {
        it("should return an IterableIterator of keys", () => {
            const dictionary = new ReadonlyDictionary(new Dictionary(null, Enumerable.range(1, 100).toDictionary(x => x, x => 1/x)));
            let index = 1;
