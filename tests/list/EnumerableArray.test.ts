...
import {EnumerableArray} from "../../src/list/EnumerableArray";
...
describe("EnumerableArray", () => {
    ...
    describe("#add()", () => {
        const list = new EnumerableArray([1, 2, 3]);
        it("should add element at the end of the list", () => {
            list.add(4);
            expect(list.size()).to.eq(4);
            expect(list.get(3)).to.eq(4);
            expect(list[3]).to.eq(4);
            expect(list[0]).to.eq(list.get(0));
            expect(list[1]).to.eq(list.get(1));
            expect(list[2]).to.eq(list.get(2));
            expect(list[3]).to.eq(list.get(3));
    ...
    describe("#addAll()", () => {
        it("should add all elements from the second list to this list", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla]);
            const list2 = new EnumerableArray([Person.Vanessa, Person.Viola]);
        ...
        it("should return true if list is changed as a result (and false if not)", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla]);
            const list2 = new EnumerableArray([Person.Vanessa, Person.Viola]);
            const list3 = new EnumerableArray<Person>();
    ...
    describe("#addAt()", () => {
        it("should throw error if index is out of bounds", () => {
            const list = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla]);
        ...
        it("should add the element to the specified index", () => {
            ...
            expect(list[2]).to.eq(Person.Bella);
        ...
        it("should not throw error and add the element if index is equal to size", () => {
            ...
            expect(list[4]).to.eq(Person.Bella);
            expect(list.length).to.eq(5);
        });
    });
    ...
    describe("#aggregate()", () => {
        ...
        it("should throw error if list is empty and no seed is provided", () => {
            const list = new EnumerableArray<number>([]);
    ...
    describe("#all()", () => {
        const list = new EnumerableArray([Person.Alice, Person.Mel, Person.Senna, null, Person.Jane]);
    ...
    describe("#append()", () => {
        it("should append the given element at the end of enumerable", () => {
            const list = new EnumerableArray([1, 2, 3, 4, 5]);
    ...
    describe("#average()", () => {
        it("should return 99948748093", () => {
            const list = new EnumerableArray(["10007", "37", "299846234235"]);
        ...
        it("should throw error if list is empty", () => {
            const list = new EnumerableArray<string>([]);
            expect(() => list.average(s => parseInt(s, 10))).to.throw(ErrorMessages.NoElements);
        });
    });

    describe("#clear()", () => {
        const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla, Person.Vanessa, Person.Viola]);
    ...
    describe("#concat()", () => {
        it("should return a list with [1,2,3,4,5,5,6,7,8,9]", () => {
            const list1 = new EnumerableArray([1, 2, 3, 4, 5]);
            const list2 = new EnumerableArray([5, 6, 7, 8, 9]);
    ...
    describe("#contains()", () => {
        const list = new EnumerableArray([1, 3, 5, 6, 7, 8, 9, 2, 0, -1, 99, -99]);
        const personList = new EnumerableArray([Person.Alice, Person.Mel, Person.Senna]);
        ...
        it("should run fast with big collections", () => {
            ...
            const list = new EnumerableArray(bigPeopleArray);
    ...
    describe("#count()", () => {
        it("should return 2", () => {
            const list = new EnumerableArray<Person>();
        ...
        it("should return 5", () => {
            const list = new EnumerableArray([1, 9, 2, 8, 3, 7, 4, 6, 5, 0]);
    ...
    describe("#defaultIfEmpty()", () => {
        it("should return a new IEnumerable with the default values", () => {
            const list = new EnumerableArray();
        ...
        it("should disregard the given value if list is not empty", () => {
            const list = new EnumerableArray([1]);
    ...
    describe("#distinct()", () => {
        it("should remove duplicate elements", () => {
            const list = new EnumerableArray([Person.Alice, Person.Mel, Person.Senna, Person.Mel, Person.Alice]);
        ...
        it("should use default comparator if no comparator is provided", () => {
            const list1 = new EnumerableArray([1, 2, 3, 1, 1, 1, 4, 5, 4, 3]);
            const list2 = new EnumerableArray(["Alice", "Vanessa", "Misaki", "Alice", "Misaki", "Megumi", "Megumi"]);
        ...
        it("should use provided comparator for key comparison", () => {
            const list2 = new EnumerableArray([Person.Alice, Person.Hanna, Person.Hanna2, Person.Noemi, Person.Noemi2]);
    ...
    describe("#elementAt()", () => {
        const list = new EnumerableArray([1, 48, 6, 195, 47]);
        it("should return 48", () => {
            const item = list.elementAt(1);
            expect(item).to.eq(48);
            expect(list[1]).to.eq(list.elementAt(1));
        });
        it("should throw error if index is out of bounds", () => {
    ...
    describe("#entries()", () => {
        it("should return an indexed IterableIterator", () => {
            const list = new EnumerableArray(Enumerable.range(1, 100));
            for (const [index, element] of list.entries()) {
                expect(index + 1).to.eq(element);
                expect(list.get(index)).to.eq(element);
                expect(list[index]).to.eq(element);
            }
        });
    });
    ...
    describe("#except()", () => {
        it("should return an array of [1,2,3]", () => {
            const list1 = new EnumerableArray([1, 2, 3, 3, 4, 5]);
            const list2 = new EnumerableArray([4, 5, 6, 7, 8]);
            const elist = new EnumerableArray(list1.except(list2));
            expect(elist.toArray()).to.deep.equal([1, 2, 3]);
            expect(elist.length).to.eq(3);
        });
        it("should return an array of [1,2]", () => {
            const list1 = new EnumerableArray([1, 2, 3, 3, 4, 5]);
            const list2 = new EnumerableArray([3, 4, 5, 6, 7, 8]);
            const elist = new EnumerableArray(list1.except(list2));
            expect(elist.toArray()).to.deep.equal([1, 2]);
        });
        it("should only have 'Alice', 'Noemi' and 'Senna'", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Noemi, Person.Mel, Person.Senna, Person.Lenka, Person.Jane]);
            const list2 = new EnumerableArray([Person.Mel, Person.Lenka, Person.Jane, Person.Noemi2]);
        ...
        it("should return a set of people unique to first enumerable", () => {
            const list1 = new EnumerableArray<Person>();
            const list2 = new EnumerableArray<Person>();
    ...
    describe("#first()", () => {
        ...
        it("should return the first element if no predicate is provided.", () => {
            const list = new EnumerableArray([99, 2, 3, 4, 5]);
        ...
        it("should return a person with name 'Alice'", () => {
            const list = new EnumerableArray([Person.Mel, Person.Alice, Person.Jane]);
    ...
    describe("#get()", () => {
        ...
        it("should return the element at the specified index", () => {
            ...
            expect(list1[0]).to.eq(Person.Alice);
            expect(list1[5]).to.eq(Person.Viola);
            expect(list1[2]).to.eq(Person.Noemi);
        });
    });

    describe("#groupBy()", () => {
        const list = new EnumerableArray([Person.Alice, Person.Mel, Person.Senna, Person.Lenka, Person.Jane, Person.Karen, Person.Reina]);
        it("should group people by age", () => {
            const group = list.groupBy(p => p.age).toList();
            const ages: number[] = [];
            const groupedAges: { [age: number]: EnumerableArray<number> } = {};
            for (const ageGroup of group) {
                ages.push(ageGroup.key);
                groupedAges[ageGroup.key] ??= new EnumerableArray<number>();
                for (const pdata of ageGroup.source) {
                    groupedAges[ageGroup.key].add(pdata.age);
                }
            }
            ...
            for (const g in groupedAges) {
                ...
                expect(sameAges.select(a => a).toArray()).to.deep.equal(expectedAges);
            }
        });
    ...
    describe("#groupJoin()", () => {
        ...
        const schools = new EnumerableArray([school1, school2, school3, school4]);
        const students = new EnumerableArray([desiree, apolline, giselle, priscilla, lucrezia]);
    ...
    describe("#indexOf", () => {
        const list1 = new EnumerableArray([Person.Alice, Person.Noemi, null, Person.Noemi2, null]);
        it("should return 2", () => {
    ...
    describe("#intersect()", () => {
        ...
        it("should return an array of [3, 4, 5]", () => {
            const list1 = new EnumerableArray([1, 2, 3, 3, 4, 5, 5, 5, 11]);
            const list2 = new EnumerableArray([3, 3, 3, 4, 4, 5, 5, 6, 7, 8]);
            const elist = list1.intersect(list2).toList();
    ...
    describe("#join()", () => {
        ...
        const priscilla = new Student(400, "Priscilla", "Necci", 1);
        const lucrezia = new Student(500, "Lucrezia", "Volpe", 4);
        const schools = new EnumerableArray([school1, school2, school3]);
        const students = new EnumerableArray([desiree, apolline, giselle, priscilla, lucrezia]);
        ...
        it("should join key-value pairs", () => {
            const pairList1 = new EnumerableArray([new Pair(1, "A"), new Pair(2, "B"), new Pair(3, "C")]);
            const pairList2 = new EnumerableArray([new Pair(1, "a1"), new Pair(1, "a2"), new Pair(1, "a3"), new Pair(2, "b1"), new Pair(2, "b2")]);
    ...
    describe("#last()", () => {
        ...
        it("should return 87", () => {
            const list = new EnumerableArray([9, 34, 65, 92, 87, 435, 3, 54, 83, 23, 87, 67, 12, 19]);
    ...
    describe("#max()", () => {
        const list = new EnumerableArray([43, 56, 123, 65, 1, 6, 900, 2312, 555, 1011]);
        ...
        it("should return 23", () => {
            const personList = new EnumerableArray([Person.Alice, Person.Mel, Person.Senna, Person.Lenka, Person.Jane]);
    ...
    describe("#orderBy()", () => {
        it("should order people by age [asc]", () => {
            const people = new EnumerableArray([Person.Alice, Person.Lenka, Person.Jane, Person.Jisu, Person.Karen, Person.Mel, Person.Rebecca, Person.Reina, Person.Senna, Person.Vanessa, Person.Viola]);
    ...
    describe("#prepend()", () => {
        ...
        it("should return a new list", () => {
            const newList = new EnumerableArray(list.prepend(Person.Lenka));
            expect(list.size()).to.eq(2);
    ...
    describe("#remove()", () => {
        ...
        it("should remove null from index 2", () => {
            ...
            expect(list1[2]).to.eq(Person.Noemi2);
        ...
        it("should remove Noemi from the list", () => {
            ...
            expect(list1[2]).to.eq(null);
            expect(list1[1]).to.eq(Person.Noemi2);
    ...
    describe("#removeAll()", () => {
        it("should remove the elements of second list from first list", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla, Person.Vanessa, Person.Viola]);
            const list2 = new EnumerableArray([Person.Vanessa, Person.Viola, Person.Noemi2]);
            list1.removeAll(list2);
            expect(list1.size()).to.eq(4);
            expect(list1.get(3)).to.eq(Person.Priscilla);
            expect(list1[3]).to.eq(Person.Priscilla);
            expect(list1.length).to.eq(4);
        });
        it("should use the provided comparator for comparison", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla, Person.Vanessa, Person.Viola], personNameComparator);
            const list2 = new EnumerableArray([Person.Vanessa, Person.Viola, Person.Noemi2]);
            list1.removeAll(list2);
            expect(list1.size()).to.eq(3);
            expect(list1.get(2)).to.eq(Person.Priscilla);
            expect(list1[2]).to.eq(Person.Priscilla);
            expect(list1.length).to.eq(3);
        });
    });
    ...
    describe("#removeAt()", () => {
        ...
        it("should remove element from the specified index", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla, Person.Vanessa, Person.Viola]);
            const removedElement = list1.removeAt(3);
            expect(list1.size()).to.eq(5);
            expect(list1.get(0)).to.eq(Person.Alice);
            expect(list1.get(4)).to.eq(Person.Viola);
            expect(list1[0]).to.eq(Person.Alice);
            expect(list1[4]).to.eq(Person.Viola);
            expect(list1.length).to.eq(5);
            expect(removedElement).to.eq(Person.Priscilla);
        });
        it("should remove element from the start index", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla, Person.Vanessa, Person.Viola]);
            const removedElement = list1.removeAt(0);
            expect(list1.size()).to.eq(5);
            expect(list1.get(0)).to.eq(Person.Lucrezia);
            expect(list1.get(4)).to.eq(Person.Viola);
            expect(list1[0]).to.eq(Person.Lucrezia);
            expect(list1[4]).to.eq(Person.Viola);
            expect(list1.length).to.eq(5);
            expect(removedElement).to.eq(Person.Alice);
        });
        it("should remove element from the end index", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Lucrezia, Person.Noemi, Person.Priscilla, Person.Vanessa, Person.Viola]);
            ...
            expect(list1[0]).to.eq(Person.Alice);
            expect(list1[4]).to.eq(Person.Vanessa);
            expect(list1.length).to.eq(5);
    ...
    describe("#retainAll()", () => {
        it("should remove elements that are not in the specified list", () => {
            const list1 = new EnumerableArray([3, 4, 1, 2, 5]);
            const list2 = new EnumerableArray([5, 4]);
    ...
    describe("#reverse()", () => {
        const list: EnumerableArray<Person> = new EnumerableArray<Person>();
        list.add(Person.Alice);
    ...
    describe("#select()", () => {
        ...
        it("should return an IEnumerable with elements [4, 25, 36, 81]", () => {
            const list = new EnumerableArray([2, 5, 6, 9]);
    ...
    describe("#selectMany()", () => {
        ...
        it("should return a flattened array of ages #1", () => {
            ...
            const peopleList = new EnumerableArray(people);
        ...
        it("should return a flattened array of ages #2", () => {
            const people: Person[] = [];
            Person.Viola.friendsList = new EnumerableArray([Person.Rebecca]);
            Person.Jisu.friendsList = new EnumerableArray([Person.Alice, Person.Mel]);
            Person.Vanessa.friendsList = new EnumerableArray([Person.Viola, Person.Rebecca, Person.Jisu, Person.Alice]);
            Person.Rebecca.friendsList = new EnumerableArray([Person.Viola]);
            people.push(Person.Viola);
            people.push(Person.Rebecca);
            people.push(Person.Jisu);
            people.push(Person.Vanessa);
    ...
    describe("#sequenceEqual()", () => {
        it("should return false for lists with different sizes", () => {
            const list1 = new EnumerableArray([1, 2]);
            const list2 = new EnumerableArray([1, 2, 3]);
            expect(list1.sequenceEqual(list2)).to.eq(false);
        });
        it("should return false if lists don't have members in the same order", () => {
            const list1 = new EnumerableArray([1, 2]);
            const list2 = new EnumerableArray([2, 1]);
            expect(list1.sequenceEqual(list2)).to.eq(false);
        });
        it("should return true if lists have members in the same order", () => {
            const list1 = new EnumerableArray([1, 2]);
            const list2 = new EnumerableArray([1, 2]);
            expect(list1.sequenceEqual(list2)).to.eq(true);
        });
        it("should return true if lists have members in the same order", () => {
            const list1 = new EnumerableArray([Person.Alice, Person.Mel, Person.Lenka, Person.Noemi]);
            const list2 = new EnumerableArray([Person.Alice, Person.Mel, Person.Lenka, Person.Noemi2]);
    ...
    describe("#singleOrDefault()", () => {
        ...
        it("should return the only element in the list", () => {
            const list2 = new EnumerableArray<string>();
            list2.add("Suzuha");
    ...
    describe("#skip()", () => {
        const list = new EnumerableArray([1, 2, 3, 4, 5, 6, 7]);
        it("should return a list with elements [5,6,7]", () => {
            const list2 = new EnumerableArray(list.skip(4));
    ...
    describe("#skipLast()", () => {
        ...
        it("should return a list with elements [1,2,3]", () => {
            const list2 = new EnumerableArray(list.skipLast(4));
            expect(list2.get(0)).to.eq(1);
            expect(list2.get(1)).to.eq(2);
            expect(list2.get(2)).to.eq(3);
            expect(list2[0]).to.eq(1);
            expect(list2[1]).to.eq(2);
            expect(list2[2]).to.eq(3);
        });
        it("should return an empty list if the list contains fewer than skipped elements", () => {
    ...
    describe("#skipWhile()", () => {
        const list = new EnumerableArray([5000, 2500, 9000, 8000, 6500, 4000, 1500, 5500]);
        ...
        it("should return an IEnumerable with elements [4000, 1500, 5500]", () => {
            const list2 = new EnumerableArray(list.skipWhile((n, nx) => n > nx * 1000));
            expect(list2.get(0)).to.eq(4000);
            expect(list2.get(1)).to.eq(1500);
            expect(list2.get(2)).to.eq(5500);
            expect(list2[0]).to.eq(4000);
            expect(list2[1]).to.eq(1500);
            expect(list2[2]).to.eq(5500);
    ...
    describe("#sort", () => {
        it("should sort list with the specified constructor", () => {
            const list = new EnumerableArray([Person.Noemi, Person.Alice, Person.Lucrezia, Person.Priscilla, Person.Eliza, Person.Bella]);
            ...
            expect(list[0]).to.eq(Person.Alice);
            expect(list[1]).to.eq(Person.Bella);
            expect(list[2]).to.eq(Person.Eliza);
            expect(list[3]).to.eq(Person.Lucrezia);
            expect(list[4]).to.eq(Person.Noemi);
            expect(list[5]).to.eq(Person.Priscilla);
        });
        it("should sort list with the default comparator if no comparator is specified", () => {
            const list = new EnumerableArray([3, 1, -1, 8, 10, -9]);
            list.sort();
            ...
            expect(list[0]).to.eq(-9);
            expect(list[1]).to.eq(-1);
            expect(list[2]).to.eq(1);
            expect(list[3]).to.eq(3);
            expect(list[4]).to.eq(8);
            expect(list[5]).to.eq(10);
        })
    });

    describe("#sum()", () => {
        it("should return 21", () => {
            const list = new EnumerableArray([1, 2, 3, 4, 5, 6]);
    ...
    describe("#takeWhile()", () => {
        const list = new EnumerableArray(["apple", "banana", "mango", "orange", "plum", "grape"]);
    ...
    describe("#thenBy()", () => {
        ...
        it("should order people by age [asc] then by name[asc] then by surname[asc]", () => {
            const people = new EnumerableArray([Person.Bella, Person.Amy, Person.Emily, Person.Eliza, Person.Hanna, Person.Hanna2,
    ...
    describe("#toLookup()", () => {
        const list = new EnumerableArray([Person.Suzuha, Person.Suzuha2, Person.Suzuha3, Person.Noemi, Person.Noemi2, Person.Hanna, Person.Hanna2]);
    ...
    describe("#toSortedDictionary()", () => {
        const people = new EnumerableArray([Person.Alice, Person.Vanessa, Person.Viola, Person.Lenka, Person.Senna]);
    ...
    describe("#union()", () => {
        it("should return a set of items from two lists", () => {
            const list1 = new EnumerableArray([1, 2, 3, 4, 5, 5, 5]);
            const list2 = new EnumerableArray([4, 5, 6, 7, 8, 9, 7]);
            const union = list1.union(list2, (n1, n2) => n1 === n2);
            expect(union.toArray()).to.deep.equal([1, 2, 3, 4, 5, 6, 7, 8, 9]);
        });
        it("should use default comparator if no comparator is provided", () => {
            const list1 = new EnumerableArray(["Alice", "Misaki", "Megumi", "Misaki"]);
            const list2 = new EnumerableArray(["Alice", "Rei", "Vanessa", "Vanessa", "Yuzuha"]);
    ...
    describe("#zip()", () => {
        const numberList = new EnumerableArray([1, 2, 3, 4]);
        const stringList = new EnumerableArray(["one", "two", "three"]);
        ...
        it("should return array of tuple if predicate is null", () => {
            const list = new EnumerableArray([2, 5, 6, 99]);
            const list2 = new EnumerableArray([true, true, false, true]);
